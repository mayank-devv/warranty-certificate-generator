...
st.caption("Upload your DOCX template, fill the form, and generate a formatted warranty certificate.")
...
# Helper: add blue line
def add_horizontal_line(paragraph):
    p = paragraph._p
    pPr = p.get_or_add_pPr()
    pBdr = parse_xml(
        r'<w:pBdr %s><w:bottom w:val="single" w:sz="4" w:space="1" w:color="0070C0"/></w:pBdr>'
        % nsdecls("w")
    )
    pPr.append(pBdr)

# Merge and style replacements
...
if submitted:
    if not template_file:
        ...
        # ---- controlled line placement ----
        # 1️⃣ After Customer/Date
        for i, p in enumerate(doc.paragraphs):
            if "Customer" in p.text:
                add_horizontal_line(p)
                break

        # 2️⃣ After GEM Contract No
        for i, p in enumerate(doc.paragraphs):
            if "GEM Contract No" in p.text:
                add_horizontal_line(p)
                break

        # 3️⃣ Before Supplied Product Details
        for i, p in enumerate(doc.paragraphs):
        ...
        st.success("✅ Warranty Certificate formatted and ready.")
        st.download_button(
